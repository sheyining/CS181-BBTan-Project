...
def transferMapPosToFigurePos(m, n):
    """把map的位置转为像素点的坐标  side指每一个小框框的边长，可能还需要加margin长度以及各种长度，最后应该得出的是在canvas里的相对位置（因为我们的startx是cancas里的相对位置?如果能转化成绝对位置也行"""
    a = (m+0.5)*side 
    b = (n+0.5)*side
    return a, b
...
def getFeatures(state, action):
    ...
    feature['angleDiff1'] = math.fabs(shootingAngle - angleCalculator(maxValueAtFigureX, maxValueAtFigureY, startPosX, startPosY))

    #feature2：计算最接近底层的小框框所在的位置，sumof角度差值
    angleDiff2 = 0
    lowestPosList = []
    lowestRow = -1
